...
class RadarCubeReader:
    ...
    def _parse_frame(self, raw_frame_bytes: bytes) -> RadarCube:
        """
        ...
        try:
            ...
            # reshape 1D cmplx_data_flat array into RadarCube data format
            # MMWAVE-L-SDK format is Cube[chirp][antenna][range]
            radar_cube_data = cmplx_data_flat.reshape((
                self.num_doppler_chirps,
                self.num_virt_antennas,
                self.num_range_bins
            ))

            # create RadarCube object and return it
            cube = RadarCube(radar_cube_data)
            return cube
